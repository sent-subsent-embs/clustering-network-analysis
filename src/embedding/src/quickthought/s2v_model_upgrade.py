...
def read_vocab_embs(vocabulary_file, embedding_matrix_file):
  ...
  with tf.compat.v1.gfile.GFile(vocabulary_file, mode="r") as f:
...
class s2v(object):
  ...
  def build_inputs(self):
    ...
    if self.mode == "encode":
      encode_ids = tf.compat.v1.placeholder(tf.int64, (None, None), name="encode_ids")
  ...
  def _initialize_cell(self, num_units, cell_type="GRU"):
    if cell_type == "GRU":
      return tf.compat.v1.nn.rnn_cell.GRUCell(num_units=num_units)
    elif cell_type == "LSTM":
      return tf.compat.v1.nn.rnn_cell.LSTMCell(num_units=num_units)
    else:
  ...
  def bow(self, word_embs, mask):
    ...
    bow = tf.reduce_sum(input_tensor=word_embs_mask, axis=1)
    return bow

  def rnn(self, word_embs, mask, scope, encoder_dim, cell_type="GRU"):

    length = tf.compat.v1.to_int32(tf.reduce_sum(input_tensor=mask, axis=1), name="length")
  ...
  def build_encoder(self):
    ...
    for i in range(2):
      with tf.compat.v1.variable_scope("encoder" + names[i]) as scope:
  ...
  def build_loss(self):
    ...
    if FLAGS.dropout:
      ...
      bin_mask = tf.random.uniform(mask_shp) > FLAGS.dropout_rate
      bin_mask = tf.compat.v1.where(bin_mask, tf.ones(mask_shp), tf.zeros(mask_shp))
    ...
    scores = tf.linalg.set_diag(scores, np.zeros(FLAGS.batch_size))
    ...
    losses = tf.nn.softmax_cross_entropy_with_logits(
        labels=tf.stop_gradient(targets), logits=scores)
  
    loss = tf.reduce_mean(input_tensor=losses)

    tf.compat.v1.summary.scalar("losses/ent_loss", loss)
    self.total_loss = loss

    if self.mode == "eval":
      f_max = tf.cast(tf.argmax(input=f_scores, axis=1), dtype=tf.int64)
      b_max = tf.cast(tf.argmax(input=b_scores, axis=1), dtype=tf.int64)
      ...
      for name, value in names_to_values.iteritems():
        tf.compat.v1.summary.scalar(name, value)
